...
func parsePieces(fileLen int, pieces string, pieceLength int) ([]Piece, error) {
	requiredNumOfPieces := int(math.Ceil(float64(fileLen) / float64(pieceLength)))
	piecesLen := len(pieces)
	numOfPieces := piecesLen / sha1.Size

	if piecesLen%sha1.Size != 0 {
		return nil, fmt.Errorf("pieces length must be a multiple of %d", sha1.Size)
	}

	if numOfPieces < requiredNumOfPieces {
		return nil, fmt.Errorf("there are not enough pieces for all files in the files list")
	}

	piecesArr := make([]Piece, numOfPieces)

	for i, j := 0, 0; i < piecesLen; i += sha1.Size {
		pieceHash := []byte(pieces[i : sha1.Size+i])
		...
		if j == numOfPieces-1 {
			piece.Length = fileLen - (pieceLength * (numOfPieces - 1))
		} else {
