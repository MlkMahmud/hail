...
func parseFilesList(infoDict map[string]any) (*torrentInfo, error) {
	...
	for fileIndex, fileEntry := range filesList {
		...
		cumulativeFilesLength += entry.length
		...
		for index := pieceStartIndex; index <= pieceEndIndex; index++ {
			if reflect.ValueOf(pieces[index]).IsZero() {
				...
				*/
				if isLastPiece {
					pieces[index].length = cumulativeFilesLength - (pieceLength * (numOfPieces - 1))
				} else {
	...
	if cumulativeFilesLength != cumulativePiecesLength {
		return nil, fmt.Errorf("total length of all files (%d) does not match the total length of all pieces (%d)", cumulativeFilesLength, cumulativePiecesLength)
	}

	return &torrentInfo{
		files:       files,
		length:      cumulativeFilesLength,
